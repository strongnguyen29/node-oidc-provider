...
export default (provider) => class Session extends hasFormat(provider, 'Session', instance(provider).BaseModel) {
  ...
  static async get(ctx) {
    ...
    // fork: deviceId do middleware của app đặt lên ctx.req; thư viện không tự
    // suy ra. Phải là provider.use() ở phía app, không phải app.use(): use()
    // splice middleware vào trước middleware nội bộ nên chạy trước Session.get.
    if (ctx.req.deviceId) {
  ...
  authorizationFor(clientId, value) {
    // the call will not set, let's not modify the session object
    if (value === undefined && !this.authorizations) {
      return {};
    }
  ...
  sidFor(clientId, value) {
    const authorization = this.authorizationFor(clientId, value);
