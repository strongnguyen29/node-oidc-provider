...
class Provider extends events.EventEmitter {
  ...
  registerGrantType(name, handler, params, dupes) {
    ...
    const defaultParams = instance(this).configuration('grantTypeParamsDefault');
    if (defaultParams) {
      grantParams.add(Set.prototype.add.bind(defaultParams));
    }
  ...
  cookieName(type) {
    ...
    const prefix = instance(this).configuration('cookies.prefix');
