...
function makeDefaults() {
  const defaults = {
     ...
     *
     * title: Additional Parameters for Every Registered Grant Type
     *
     ...
     *   These parameters are injected in addition to the ones a grant type declares for itself,
     *   and become available in `ctx.oidc.params` inside the grant type handler.
     *
     * Note: Only grant types registered after this configuration is in place are affected, which
     *   is every grant type registered through `registerGrantType` since the configuration is
     *   read at registration time. The built-in grant types are unaffected.
     *
     * Note: This is a fork-specific extension, it is not part of upstream oidc-provider.
     *
     ...
     *
     * Note: Passing the scope check is not the same as receiving claims. The claims returned are
     *   still filtered by the granted OpenID Connect scopes, so an access token that carries only
     *   a non-OpenID scope from this list is accepted but yields an empty response body, `sub`
     *   included.
     *
