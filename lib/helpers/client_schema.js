  ...
  class Schema {
    constructor(
      ...
      if (responseTypes.includes('token') || responseTypes.includes('id_token')) {
        if (!['implicit', 'password', 'social', 'fast_login'].some((gt) => this.grant_types.includes(gt))) {
          this.invalidate("grant_types must contain 'implicit / password / social / fast_login' when 'id_token' or 'token' are amongst response_types");
        }
      }
